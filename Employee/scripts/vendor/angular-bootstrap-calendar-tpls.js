...
/***/ function(module, exports, __webpack_require__) {
	...
	angular
	  ...
	  .controller('MwlCalendarWeekCtrl', ["$scope", "$sce", "moment", "calendarHelper", "calendarConfig", function($scope, $sce, moment, calendarHelper, calendarConfig) {
	      ...
	      $scope.createEventsTable = function (vm) {
	          vm.shift_list = calendarConfig.shift_list;
	          var shifts = null;
	          shifts = vm.shift_list;

              // using for loop instead of foreach to gain more speed
	             for (var shiftKey = 0; shiftKey < shifts.length; shiftKey++) {// loop over shifts
	          
	              shifts[shiftKey].events = [];

	              for(var daykey=0;daykey<vm.view.days.length;daykey++) { // loop over day of the week must be 7 days even if no event found

	                  event = new Object();
	                  // Value if event doesnt exist 
	                  event.exist = false;
	                  event.day = vm.view.days[daykey].date;
	                  event.startsAt = vm.view.days[daykey].date;
	                  event.status = 0; // waiting for employee validation
	                  event.shift_no = shifts[shiftKey].shift_no;
	                 
	                
	                  

	                  // loop the real event array and get the events 
	                  for(var eventkey=0;eventkey<vm.view.events.length;eventkey++ ) { 
	                      
	                      if (vm.view.events[eventkey].dayOffset == daykey && vm.view.events[eventkey].shift_no == shifts[shiftKey].shift_no) {
	                          event.id = vm.view.events[eventkey].id;
	                          event.dayOffset = vm.view.events[eventkey].dayOffset;
	                          event.daySpan = vm.view.events[eventkey].daySpan;
	                          event.shift_no = vm.view.events[eventkey].shift_no;
	                          event.startsAt = vm.view.events[eventkey].startsAt;
	                          event.status = vm.view.events[eventkey].status;
	                          event.reason_id = vm.view.events[eventkey].reason_id;
	                  ...
	                  shifts[shiftKey].events.push(event);// pushing event to array
	              }// end loop over days

	          }  // end loop over shifts
              

	             vm.shiftsAndEvents = shifts;
	             console.log(shifts);
	      };
	    ...
	    $scope.$on('calendar.refreshView', function () {
	      ...
	      $scope.createEventsTable(vm);// added
	    });
